...
import { TrainingMaterialsSidebar } from "./TrainingMaterialsSidebar";
import { TrainingMaterialsContent } from "./TrainingMaterialsContent";
...
export const TrainingMaterials: FunctionComponent = () => {
  const [fileToUpload, setFileToUpload] = useState<FileList | null>();
  const [fileList, setFileList] = useState<Models.FileList>();
  ...
  useEffect(() => {
    const getFilesFromBucket = async () => {
      const result = await storage.listFiles("6524bd1e14ebf786d09e");
      setFileList(result);
    };
    getFilesFromBucket();
  }, [trigger]);
  ...
  return (
    <div className="h-[calc(100vh-5rem)] flex items-center">
      {/* <div
        role="button"
        className="rounded-xl border-4 w-full h-44 border-dashed flex items-center justify-center dark:border-zinc-700 dark:bg-zinc-600/5 relative"
      >
        <input
          //   ref={inputRef}
          type="file"
          className="border absolute w-full h-full opacity-0 cursor-pointer"
          onChange={(e) => setFileToUpload(e.target.files)}
        />

        <div className="flex flex-col items-center gap-2">
          {fileToUpload === undefined ? (
            <h3 className="text-4xl font-bold dark:text-zinc-600 tracking-wide">
              Upload a new file
            </h3>
          ) : (
            <div className="flex flex-col items-center gap-3">
              <p className="text-2xl">{fileToUpload?.[0].name}</p>
              <button
                className="dark:bg-indigo-700 px-5 py-2 rounded z-20"
                onClick={async (e) => {
                  e.stopPropagation();
                  await storage.createFile(
                    "6524bd1e14ebf786d09e",
                    nanoid(20),
                    //@ts-ignore
                    fileToUpload[0]
                  );
      ...
      {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-y-5 gap-x-7 md:gap-y-7 lg:gap-y-7 xl:gap-y-7">
        {fileList?.files.map((file, index) => {
          console.log(file.mimeType);

          const preview = storage.getFilePreview("6524bd1e14ebf786d09e", file.$id);

          return (
            <div
              key={index}
              className="flex flex-col items-center hover:dark:bg-zinc-900/50 p-4 rounded-lg gap-3"
            >
              {file.mimeType === MimeTypes.JPEG || file.mimeType === MimeTypes.PNG ? (
                <Image
                  width={300}
                  height={300}
                  src={preview.href}
                  alt=""
                  className="h-40 w-full object-cover rounded shrink-0"
                />
              ) : (
                <Thumbnail mimeType={file.mimeType} />
              )}
              <div className="w-full px-10 flex flex-col items-center">
                <h3 className="truncate max-w-[15rem] text-2xl font-semibold dark:text-zinc-300">
                  {file.name}
                </h3>
      ...
      <TrainingMaterialsSidebar />
      <TrainingMaterialsContent />
    </div>
  );
};

const Thumbnail: FunctionComponent<ThumbnailProps> = ({ mimeType }) => {
  return (
    <div className="h-40 rounded-xl overflow-clip shrink-0">
